...
- Seuil d'abstention **calibré empiriquement**, pas deviné. Sur le jeu élargi à 62 questions, les bandes de score **se recouvrent** : une question administrative hors périmètre (« conditions pour obtenir un passeport marocain », 0.603) dépasse la question de droit du travail la moins bien classée (0.596). Aucun seuil unique ne peut donc les séparer — la séparation nette qu'affichait le jeu d'origine, avec ses cinq seules questions hors périmètre, était un artefact. Seuil maintenu à 0.42 pour ne filtrer que la bande clairement non pertinente ; le reste est rattrapé par l'abstention au niveau du prompt.
- `eval/reference_qa.jsonl` (62 questions) + `eval/run_eval.py` : Recall@5 / MRR mesurés — dense seul 0.849/0.778, BM25 seul 0.660/0.535, **hybride 0.868/0.775**.

  > ⚠️ **Ce constat a été corrigé.** Une version antérieure de ce document annonçait « dense seul 0.969/0.891 » et concluait que le dense dépassait l'hybride. C'était mesuré sur 37 questions **toutes composées de reformulations en langage courant** — la configuration exactement favorable au sémantique. Un audit de couverture a montré qu'aucune de ces questions ne nommait un article par son numéro et que trois Livres sur huit n'étaient pas couverts. Après élargissement ciblé à 62 questions, l'hybride repasse devant, sans qu'une ligne du moteur ait changé. Détail dans `JOURNAL.md` et au chapitre 5 du rapport.
