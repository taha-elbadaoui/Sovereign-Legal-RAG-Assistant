...
\begin{figure}[H]
    ...
    \caption{Architecture générale du système : pipeline d'indexation (hors ligne) et pipeline
    de requête (en ligne). Les index construits hors ligne (traits pleins) sont interrogés à
    chaque question (traits pointillés) ; le modèle d'embedding BGE-M3 (en vert) intervient aux
    deux étapes, ce qui garantit que la question et les articles vivent dans le même espace
    vectoriel.}
...
requête, pour vectoriser la question. C'est cette identité qui donne un sens à la comparaison
entre les deux.
...
\begin{enumerate}
    ...
    les deux classements sont fusionnés par RRF.
    ...
    \item Sinon, les articles retenus sont transmis au générateur, qui construit un prompt
    strict et appelle le modèle local à basse température.
    \item Le système \textbf{vérifie que chaque numéro d'article cité figure bien dans le
    contexte fourni}, puis affiche la réponse, les articles cités et l'avertissement d'usage.
\end{enumerate}
...
sans toucher aux autres : changer de modèle de langage n'affecte que \texttt{generator.py} ;
passer du dense seul à l'hybride n'affecte que \texttt{retriever.py}. Cette contrainte est
délibérée : elle rend l'\textbf{évaluation comparative} possible, en permettant de ne mesurer
qu'une variante à la fois. Seul le modèle d'embeddings fait exception : il est partagé entre
\texttt{database.py} et \texttt{retriever.py}, les deux devant impérativement utiliser le même.
...
\hline
Runtime LLM & Ollama & Une commande télécharge et sert un modèle quantifié via une API locale propre. \\
\hline
