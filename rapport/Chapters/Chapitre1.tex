...
l'avance (figure~\ref{fig:gantt}). Le découpage suit une progression délibérée : rendre le corpus
fiable avant de construire quoi que ce soit dessus, obtenir une chaîne complète avant de
l'optimiser, et mesurer avant de conclure.

\begin{figure}[htbp]
  \centering
  \input{Figures/fig-gantt}
  \caption[Phasage prévu du stage]{Phasage \emph{prévu} du stage. Les jalons marquent les moments
  où le travail devenait vérifiable par un tiers : un corpus qui se régénère à l'identique, des
  réponses effectivement citées, un banc de mesure rejouable.}
  \label{fig:gantt}
\end{figure}

\begin{constat}
L'exécution réelle s'est écartée de ce plan, et l'écart est instructif. Le journal de bord montre
que les trois premières semaines (2 au 20~juillet) ont été absorbées par le \emph{seul} corpus ---
extraction, découpage, hiérarchie, correction des trois familles de défauts décrites au
chapitre~\ref{chap:realisation} --- tandis que les phases S3 à~S6, du premier pipeline RAG jusqu'à
l'évaluation complète, ont été menées en deux jours, les 24 et 25~juillet.

Ce déséquilibre n'est pas un accident de planification : il est la manifestation directe de la
thèse défendue dans ce rapport. Une fois le corpus fiable, le moteur RAG se construit vite ; c'est
la donnée qui coûte, pas la boucle qui l'exploite.
\end{constat}
